...
def evaluate_cmb(model_path, dataset_path, num_samples=100):
    ...
    if is_peft:
        # LoRA Adapter 需要先加载基座，再挂载 Adapter
        import json
...
def main():
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(os.path.dirname(__file__), "../../outputs/cmb_eval_ft_comparison.png"))
    print("\nCMB 评估完成，结果已保存到 eval_results_cmb.json，并生成图表 cmb_eval_ft_comparison.png")
