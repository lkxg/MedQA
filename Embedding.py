...
triples_set = set()
with open('./medical_kg_data/triples.tsv', 'r', encoding='utf-8') as f:
    for line in f:
...
# 保存嵌入
torch.save(entity_emb_real.float(), 'entity_embeddings.pt')

relation_emb_real = relation_emb.real if relation_emb.is_complex() else relation_emb
torch.save(relation_emb_real.float(), 'relation_embeddings.pt')

with open('entity_to_id.json', 'w', encoding='utf-8') as f:
    json.dump(dict(entity_to_id), f, ensure_ascii=False, indent=2)
with open('relation_to_id.json', 'w', encoding='utf-8') as f:
    json.dump(dict(tf.relation_to_id), f, ensure_ascii=False, indent=2)

# ⭐ 保存完整模型 checkpoint（避免重复训练）
checkpoint_dir = './rotate_checkpoint'
...
print("\n✅ 全部完成！")
print(f"   嵌入: entity_embeddings.pt ({entity_emb_real.shape})")
